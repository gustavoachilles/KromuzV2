...
import {
  ...
  Filter, CheckSquare, Square, ListChecks, Pin, PinOff, Ban, Mail, MailOpen
} from "lucide-react";
...
const ABAS = [
  ...
  { key: "ARQUIVADA", label: "Arquivadas", icon: Archive },
];
...
export function InboxClient({ conversas: initConversas, sessao }: { conversas: any[]; sessao: any }) {
  ...
  const filtradas = useMemo(() => {
    return conversas.filter(c => {
      // Aba "Arquivadas" mostra só arquivadas
      if (abaAtiva === "ARQUIVADA") return c.arquivada === true;
      // Nas outras abas, esconder arquivadas
      if (abaAtiva !== "ARQUIVADA" && c.arquivada) return false;
  ...
  const contadores = useMemo(() => ({
    "": conversas.filter(c => !c.arquivada).length,
    AGUARDANDO_IA: conversas.filter(c => c.status === "AGUARDANDO_IA" && !c.arquivada).length,
    ABERTO: conversas.filter(c => c.status === "ABERTO" && !c.arquivada).length,
    FECHADO: conversas.filter(c => c.status === "FECHADO" && !c.arquivada).length,
    ARQUIVADA: conversas.filter(c => c.arquivada).length,
  }), [conversas]);
  ...
  // Ação genérica (para context menu — aceita qualquer convId)
  const executarAcao2 = async (convId: string, acao: string, dados?: any) => {
    try {
      const r = await fetch("/api/inbox/acoes", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ conversaId: convId, acao, dados }) });
      if (r.ok) {
        const updated = await r.json();
        setConversas(prev => prev.map(c => c.id === convId ? { ...c, ...updated } : c));
        setContextMenu(null);
        toast.success("OK!");
      }
    } catch { toast.error("Erro."); }
    ...
    <div className="h-screen py-4 pr-4">
      <div className="flex h-full bg-white dark:bg-zinc-950 rounded-3xl border border-zinc-200 dark:border-zinc-800 overflow-hidden shadow-sm">
        ...
        <div className="w-80 border-r border-zinc-200 dark:border-zinc-800 flex flex-col bg-zinc-50/50 dark:bg-zinc-900/30">
          ...
          <div className="flex-1 overflow-y-auto" onClick={() => setContextMenu(null)}>
            {filtradas.map(conv => (
              <div key={conv.id}
                ...
                className={`flex items-stretch border-b border-zinc-100 dark:border-zinc-800/50 hover:bg-zinc-100 dark:hover:bg-zinc-800 transition ${ativaId === conv.id ? "bg-brand/10 dark:bg-brand/20" : ""}`}>
                {modoSelecao && (
                ...
                <button onClick={() => { if (!modoSelecao) setAtivaId(conv.id); else toggleSelecao(conv.id); }}
                  ...
                  <div className="flex justify-between items-start mb-1">
                    <div className="flex items-center gap-2 flex-1 min-w-0">
                      <div className={`w-8 h-8 rounded-full flex items-center justify-center text-xs font-bold shrink-0 ${conv.bloqueada ? "bg-red-100 dark:bg-red-900/30 text-red-500" : "bg-brand/10 dark:bg-brand/20 text-brand"}`}>
                        {conv.bloqueada ? <Ban className="w-4 h-4" /> : conv.clienteNome.substring(0, 2).toUpperCase()}
                      </div>
                      <div className="min-w-0 flex-1">
                        <span className="font-semibold text-sm text-zinc-900 dark:text-zinc-100 truncate flex items-center gap-1">
                          {conv.fixada && <Pin className="w-3 h-3 text-amber-500 shrink-0" />}
                          {conv.clienteNome}
                        </span>
          ...
          {contextMenu && (() => {
            const conv = conversas.find(c => c.id === contextMenu.convId);
            if (!conv) return null;
            return (
              <div className="fixed z-[999] bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-xl shadow-2xl py-1 w-48"
                style={{ left: contextMenu.x, top: contextMenu.y }}
                onClick={() => setContextMenu(null)}>
                <button onClick={() => executarAcao2(contextMenu.convId, conv.fixada ? "DESFIXAR" : "FIXAR")} className="w-full text-left px-3 py-2 text-xs flex items-center gap-2 hover:bg-zinc-50 dark:hover:bg-zinc-800">
                  {conv.fixada ? <PinOff className="w-3.5 h-3.5 text-amber-500" /> : <Pin className="w-3.5 h-3.5 text-amber-500" />}
                  {conv.fixada ? "Desafixar" : "Fixar no topo"}
                </button>
                <button onClick={() => executarAcao2(contextMenu.convId, "MARCAR_NAO_LIDO")} className="w-full text-left px-3 py-2 text-xs flex items-center gap-2 hover:bg-zinc-50 dark:hover:bg-zinc-800">
                  <Mail className="w-3.5 h-3.5 text-blue-500" /> Marcar não lido
                </button>
                <button onClick={() => executarAcao2(contextMenu.convId, conv.arquivada ? "DESARQUIVAR" : "ARQUIVAR")} className="w-full text-left px-3 py-2 text-xs flex items-center gap-2 hover:bg-zinc-50 dark:hover:bg-zinc-800">
                  <Archive className="w-3.5 h-3.5 text-zinc-500" />
                  {conv.arquivada ? "Desarquivar" : "Arquivar"}
                </button>
                <div className="border-t border-zinc-100 dark:border-zinc-800 my-1" />
                <button onClick={() => executarAcao2(contextMenu.convId, conv.bloqueada ? "DESBLOQUEAR" : "BLOQUEAR")} className="w-full text-left px-3 py-2 text-xs flex items-center gap-2 hover:bg-zinc-50 dark:hover:bg-zinc-800 text-red-500">
                  <Ban className="w-3.5 h-3.5" />
                  {conv.bloqueada ? "Desbloquear" : "Bloquear contato"}
                </button>
              </div>
            );
          })()}
        ...
        <div className="flex-1 flex flex-col bg-[#efeae2] dark:bg-zinc-950 relative">
          {conversaAtiva ? (
            <>
              ...
              {/* Barra de tipo */}
              {conversaAtiva.bloqueada && (
                <div className="px-4 py-3 bg-red-50 dark:bg-red-950/30 border-t border-red-200 dark:border-red-800 flex items-center gap-2 text-red-600 text-sm font-semibold flex-shrink-0">
                  <Ban className="w-4 h-4" /> Contato bloqueado — mensagens desativadas
                  <button onClick={() => executarAcao("DESBLOQUEAR")} className="ml-auto text-xs bg-red-100 dark:bg-red-900/50 px-2 py-1 rounded font-bold hover:bg-red-200">Desbloquear</button>
                </div>
              )}
              {conversaAtiva.status !== "FECHADO" && !conversaAtiva.bloqueada && (
                <>
