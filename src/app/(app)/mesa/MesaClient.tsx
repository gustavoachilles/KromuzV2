...
export function MesaClient({ sessao, propostas, leadsHoje, ultimasPropostas=[], kpis }: {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-zinc-50 to-white dark:from-zinc-950 dark:to-black">
      ...
      <NovaPropostaModal
        open={novaPropostaModal}
        onClose={() => setNovaPropostaModal(false)}
        onSuccess={(p: any) => { setNovaPropostaModal(false); router.push(`/esteira?proposta=${p.id}`); }}
        bancos={npBancos}
        convenios={npConvenios}
      />
    </div>
  );
}
