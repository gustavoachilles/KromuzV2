      ...
      <div className="max-w-7xl mx-auto px-6 py-10 space-y-8">
        ...
        {vendedores.length === 0 ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
            {vendedores.map((v, i) => (
              <div key={v.email} className={`rounded-2xl border p-6 transition hover:shadow-md ${i === 0 ? "border-brand shadow-lg shadow-brand/10 bg-gradient-to-br from-brand/5 to-white dark:from-brand/10 dark:to-zinc-900" : "border-zinc-200 dark:border-zinc-800 bg-white dark:bg-zinc-900"}`}>
                ...
                <div className="grid grid-cols-2 gap-3">
                  <MiniStat icon={<TrendingUp className="h-3.5 w-3.5 text-brand" />} label="Volume" value={fmt(v.volume)} sub={`${fmt(v.volumePago)} pago`} />
                  ...
                  <MiniStat icon={<Users className="h-3.5 w-3.5 text-brand" />} label="Leads" value={`${v.totalLeads}`} sub={`${v.leadsNovos} novos`} />
                </div>
                ...
                {v.meta && (
                  <div className="mt-4 pt-3 border-t border-zinc-100 dark:border-zinc-800">
                    ...
                    <div className="grid grid-cols-2 gap-2">
                      {v.meta.metaVolume && <MetaBar label="Volume" real={v.volumePago} meta={v.meta.metaVolume} color="bg-brand" />}
